...
export default function Home() {
  ...
  // --- HÀM RENDERING ICON SVG THEO ĐƯỜNG DẪN URL CỦA BẠN ---
  const renderSocialIconRaw = (link) => {
    if (link.includes("github.com")) {
      ...
      return (
        <svg className="w-5 h-5 hover:text-blue-600 transition-colors" fill="currentColor" viewBox="0 0 24 24">
          <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
        </svg>
      );
    }
    // Icon quả cầu mặc định cho các link khác ngoài GitHub/LinkedIn
    return (
      <svg className="w-5 h-5 hover:text-gray-900 transition-colors" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
        ...
        <header className="max-w-6xl mx-auto px-4 py-16 md:py-24 grid md:grid-cols-3 gap-8 items-center">
          <div className="md:col-span-2 space-y-6">
            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight">
              Hi, I'm {personalDetail?.fullName || "Quoc Toan"} <br />
              <span className="text-gray-500">{personalDetail?.title || "Software Engineer"}</span>
            </h1>
